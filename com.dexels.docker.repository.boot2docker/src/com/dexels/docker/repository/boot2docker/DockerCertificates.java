...
/**
 * DockerCertificates holds certificates for connecting to an HTTPS-secured
 * Docker instance with client/server authentication.
 */
public class DockerCertificates {
	...
	private static final char[] KEY_STORE_PASSWORD = "docker!!11!!one!"
			.toCharArray();

	final SSLContext sslContext;

	public DockerCertificates(final Path dockerCertPath)
	...
	private DockerCertificates(final Builder builder)
			throws DockerCertificateException {
		try {
			final CertificateFactory cf = CertificateFactory
					.getInstance("X.509");
			final Certificate caCert = cf.generateCertificate(Files
					.newInputStream(builder.caCertPath));
			final Certificate clientCert = cf.generateCertificate(Files
					.newInputStream(builder.clientCertPath));

			final PEMKeyPair clientKeyPair;
			try (BufferedReader newBufferedReader = Files.newBufferedReader(builder.clientKeyPath,
					Charset.defaultCharset())) {
				clientKeyPair = (PEMKeyPair) new PEMParser(
						newBufferedReader).readObject();
			...
			trustStore.setEntry("ca", new KeyStore.TrustedCertificateEntry(
					caCert), null);
			...
			this.sslContext = SSLContexts.custom()
					...
					.loadKeyMaterial(keyStore, KEY_STORE_PASSWORD).useTLS()
					.build();
	...
	public static class Builder {
		...
		public Builder dockerCertPath(final Path dockerCertPath) {
			...
			this.clientKeyPath = dockerCertPath
					.resolve(DEFAULT_CLIENT_KEY_NAME);
			this.clientCertPath = dockerCertPath
					.resolve(DEFAULT_CLIENT_CERT_NAME);
